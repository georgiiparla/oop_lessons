class Task:
    ...
    def display(self):
        print(f'{self.name} (Сделана)' if self.status is True else f'{self.name} (Не сделана)')
...
class TaskManager:
    ...
    def mark_done(self, task: Task):
        task_number = self.task_list.tasks.index(task)
        self.task_list.tasks[task_number].status = True

    def mark_undone(self, task: Task):
        task_number = self.task_list.tasks.index(task)
        self.task_list.tasks[task_number].status = False
